...
La mise en place d'un #gls("proxy", "proxy") fiable est un des piliers du projet, c'est lui qui permettra d'entraîner correctement le filtre et, plus tard, possiblement d'autres outils destinés à assister la compression. Tester différentes méthodologies de proxy était donc un premier point d'étape essentiel, et cela s'intègre naturellement dans le cadre d'un projet de fin d'études. La partie scientifique de ce proejt de fin d'études se pencehra donc en particulier ce point en apportant aussi d'autres aspects du projets mais à ce stade précoce du projet il est d'abord essentiel de comprendre les différentes possibilités avant de se définir la méthode qui sera utilisée pour l'implémentation finale, ce qui demande de réaliser des analyses comparatives.
