...
La chaîne #gls("vod", "VOD") est un processus complexe qui transforme une scène captée en une vidéo diffusée mondialement. Cette chaîne se décompose en cinq étapes majeures que l'on retrouve aussi sous forme de schéma (@vod_transmission) :
...
De ce constat, on peut estimer qu'une optimisation réaliste se trouverait en amont du transfert de la vidéo. En particulier, car une vidéo encodée sera transmise à un grand nombre d'utilisateurs, donc optimiser une vidéo pourrait alors devenir intéressant plus rapidement.
...
Afin d'introduire les analyses présentées, qui se basent sur différents outils de compression vidéo, voici un bref historique des codecs existants.
...
On voit donc que différents outils existent, chacun possède des performances de compression différentes. On peut les regrouper en deux familles selon leur modèle de licence. D'un côté, les codecs libres de redevances (royalty-free), comme VP9 ou AV1, développés notamment par l'Alliance for Open Media, utilisables sans payer de droits. De l'autre, les codecs soumis à redevances, comme H.264 ou H.265, dont l'usage suppose l'acquisition de licences auprès des détenteurs de brevets.
Ce modèle de licence pèse directement sur l'adoption. Le cas de H.265 est intéressant, sa licence, fragmentée entre plusieurs groupes de brevets, a freiné son déploiement et a précisément motivé la création d'AV1, libre de redevances. Paradoxalement, ce sont pourtant les codecs payants qui restent les plus utilisés aujourd'hui, parce qu'ils bénéficient d'un support matériel quasi universel et d'un écosystème plus mature, là où les alternatives libres, plus récentes, restent coûteuses en calcul et moins bien prises en charge par le matériel existant.

Les figures suivantes illustrent la répartition d'utilisation des outils de compression en 2023 et 2024 sur un panel d'entreprises. On peut y voir que les solutions récentes ne sont pas encore adoptées par la majorité des entreprises, cela montre bien la difficulté d'évolution.
...
#figure(
  canvas(length: 1cm, {
    import draw: *
    let inL = ((0.5, 1.0), (0.5, 2.0), (0.5, 3.0))
    let hidL = ((2.2, 0.6), (2.2, 1.53), (2.2, 2.47), (2.2, 3.4))
    let outL = ((3.9, 1.5), (3.9, 2.5))
    // connexions
    for a in inL { for b in hidL { line(a, b, stroke: rgb("#cdd6df") + 0.5pt) } }
    for a in hidL { for b in outL { line(a, b, stroke: rgb("#cdd6df") + 0.5pt) } }
    // une connexion mise en avant = un poids
    line(inL.at(1), hidL.at(2), stroke: rgb("#e8763a") + 1.4pt)
    // neurones
    for p in inL { circle(p, radius: 0.23, fill: white, stroke: rgb("#5f5e5a") + 1pt) }
    for p in hidL { circle(p, radius: 0.23, fill: white, stroke: rgb("#1f6fb2") + 1pt) }
    for p in outL { circle(p, radius: 0.23, fill: white, stroke: rgb("#2e9e5b") + 1pt) }
    // flèches entrée / sortie
    let arrow = (s, e) => line(s, e, mark: (end: ">", fill: black, scale: 0.6))
    arrow((-0.7, 2.0), (0.2, 2.0))
    arrow((4.2, 2.0), (5.1, 2.0))
    content((-0.75, 2.0), text(size: 8pt)[entrée], anchor: "east")
    content((5.15, 2.0), text(size: 8pt)[sortie], anchor: "west")
    // légende poids
    content((2.75, 2.02), text(size: 7pt, fill: rgb("#e8763a"))[un poids], anchor: "west")
    // libellés couches
    content((0.5, -0.1), text(size: 8pt)[couche d'entrée])
    content((2.2, -0.5), text(size: 8pt)[couches cachées])
    content((3.9, -0.1), text(size: 8pt)[couche de sortie])
  }),
  caption: [Un réseau de neurones : des couches de neurones reliés par des connexions, chacune portant un poids ajustable.],
) <nn_schema>

Ajuster ces poids est primordial, c'est pourquoi on parle d'apprentissage : un entraînement qui permet d'apprendre une tâche à un réseau de neurones en ajustant au mieux ces nombreux paramètres (poids).
...
#figure(
  canvas(length: 1cm, {
    ...
    let box = (cx, cy, w, h, body, fill) => {
      rect((cx - w / 2, cy - h / 2), (cx + w / 2, cy + h / 2), radius: 3pt, fill: fill, stroke: 0.7pt + luma(90))
      content((cx, cy), body)
    }
    box(1.4, 3.0, 2.4, 1.0, text(size: 8.5pt)[Exemples], rgb("#EEEDFE"))
    arrow((2.65, 3.0), (3.35, 3.0))
    box(4.9, 3.0, 2.6, 1.0, text(size: 8.5pt, weight: "bold")[Réseau], rgb("#F1EFE8"))
    arrow((6.25, 3.0), (6.95, 3.0))
    box(8.3, 3.0, 2.0, 1.0, text(size: 8.5pt)[Prédiction], rgb("#E6F1FB"))
    // résultat attendu (haut) -> erreur
    box(8.3, 5.2, 2.4, 1.0, text(size: 8pt)[Résultat attendu], white)
    arrow((9.45, 3.0), (10.35, 3.0))
    box(11.6, 3.0, 2.0, 1.0, text(size: 8.5pt, weight: "bold", fill: rgb("#BA7517"))[Erreur], rgb("#FAEEDA"))
    arrow((8.3, 4.65), (8.3, 3.55))
    content((8.55, 4.1), text(size: 7pt)[comparer], anchor: "west")
    // boucle de retour
    bezier(
      (11.6, 2.45),
      (4.9, 2.45),
      (11.0, 0.6),
      (5.5, 0.6),
      stroke: (paint: rgb("#D85A30"), thickness: 1.5pt),
      mark: (end: ">", fill: rgb("#D85A30"), scale: 0.9),
    )
    content((8.25, 0.75), text(
      size: 8pt,
      weight: "bold",
      fill: rgb("#D85A30"),
    )[ajuster les poids pour réduire l'erreur])
  }),
  caption: [La boucle d'entraînement : le réseau prédit, on mesure l'erreur par rapport au résultat attendu, puis on ajuste les poids pour la réduire.],
) <nn_train>
...
#figure(
  canvas(length: 1cm, {
    import draw: *
    let arrow = (s, e) => line(s, e, mark: (end: ">", fill: black, scale: 0.65))
    let box = (cx, cy, w, h, body, fill, strk) => {
      rect((cx - w / 2, cy - h / 2), (cx + w / 2, cy + h / 2), radius: 3pt, fill: fill, stroke: strk + 0.9pt)
      content((cx, cy), body)
    }
    let y = 3.0
    box(1.1, y, 1.9, 1.1, text(size: 8pt)[Image\ originale], white, luma(120))
    arrow((2.1, y), (2.75, y))
    box(
      4.0,
      y,
      2.3,
      1.3,
      text(size: 8pt, weight: "bold", fill: rgb("#1f6fb2"))[Filtre\ prétraitement],
      rgb("#e8f1fb"),
      rgb("#1f6fb2"),
    )
    arrow((5.2, y), (5.85, y))
    box(
      ...
      2.5,
      1.3,
      [#text(size: 8pt, weight: "bold")[Codec] #linebreak() #text(size: 7pt)[encode → décode]],
      rgb("#eef1f4"),
      luma(110),
    )
    arrow((8.7, y), (9.35, y))
    box(
      ...
      2.3,
      1.3,
      text(size: 8pt, weight: "bold", fill: rgb("#2e9e5b"))[Filtre\ post-traitement],
      rgb("#e7f6ec"),
      rgb("#2e9e5b"),
    )
    arrow((12.0, y), (12.65, y))
    box(14.0, y, 1.9, 1.1, text(size: 8pt)[Image\ affichée], white, luma(120))
    // sous-légendes
    content((4.0, y - 1.15), text(size: 7pt, fill: luma(100))[avant compression])
    content((4.0, y - 1.5), text(size: 7pt, fill: luma(100))[(côté diffuseur)])
    content((10.8, y - 1.15), text(size: 7pt, fill: luma(100))[après décompression])
    content((10.8, y - 1.5), text(size: 7pt, fill: luma(100))[(côté utilisateur)])
  }),
  caption: [Emplacement des filtres : le prétraitement (avant la compression, côté diffuseur) et le post-traitement (après la décompression, côté utilisateur)],
) <filtre_prepost>
...
Capacités est divisée en 13 cellules d'expertise. Ces cellules portent des projets d'entreprise au sein des laboratoires auxquels elles sont rattachées, afin d'avoir accès à leur expertise et de se trouver au plus près de la découverte scientifique.
...
== La cellule IXPEL
Je travaille au sein de la cellule IXPEL, intégrée à l'équipe de recherche IPI (Image Perception Interaction), qui appartient au LS2N (Laboratoire des Sciences du Numérique de Nantes). L'équipe est spécialisée dans l'intelligence artificielle appliquée à l'image et la qualité d'expérience. On y retrouve par exemple des sujets liés à l'imagerie médicale, au traitement de documents manuscrits et à l'expérience et qualité utilisateur face à du contenu vidéo, l'équipe est reconnue mondialement sur ce dernier sujet, ce qui lui permet de travailler en collaboration avec les plus grandes entreprises du secteur et en particulier avec leurs équipes de recherche.

Les clients de notre cellule sont de grandes entreprises du numérique comme Meta, Netflix ou Amazon. L'équipe IPI et la cellule IXPEL sont reconnues pour les tests subjectifs et la qualité d'expérience, c'est en partie sur ces thématiques que les projets en lien avec ces entreprises portent. Les tests permettent par exemple de recueillir des données sur la satisfaction d'utilisateurs face à des contenus vidéo, ce qui permet par la suite d'évaluer des méthodes et solutions mises en place. Nous avons également comme client le laboratoire lui-même. Quand le laboratoire montre un besoin de programmation ou d'autres tâches d'ingénierie pour un des projets en cours, il fait appel à notre cellule si cela reste dans nos domaines de compétences. D'autres clients plus ponctuels peuvent aussi faire appel à notre cellule pour la mise en place d'outils liés à la vision par ordinateur et à l'image plus généralement.
...
L'articulation globale des tâches ainsi que l'enchaînement des différents jalons de validation sont détaillés dans le diagramme de Gantt :

#align(center)[
  #figure(
    image("images/planning.png", width: 110%),
...
Pour notre cellule, on peut facilement se partager des astuces ou bonnes pratiques pour mieux appréhender ce genre d'outil. C'est devenu un indispensable pour certains membres de l'équipe, et cela permet de mieux gérer nos ressources internes. Glicid est aussi maintenu, ce qui facilite la tâche, car pour des ressources internes, cela demande plus de travail en cas de problème.

Cependant, ces maintenances imposent aussi des moments d'arrêt de l'outil, un grand nombre de jours durant ce projet ont été privés de cette ressource pour maintenance ou inaccessibilité de la plateforme notamment lors des périodes de canicule. Cela limite l'organisation des tâches et peut parfois ralentir l'avancée de certains projets.
...
Les indicateurs de réussite ont eux aussi évolué. Au départ, peu d'informations étaient disponibles pour réaliser ces choix. On s'est appuyé sur la littérature et les connaissances de l'équipe. L'avancement du projet a ensuite permis de valider ou d'ajuster ces outils d'évaluation, notamment grâce aux analyses de corrélation qui seront présentées dans le chapitre concernant l'implémentation.

L'implémentation a suivi une logique d'essais et d'erreurs, avec des tests parfois non concluants qui ont néanmoins permis de mieux comprendre les mécanismes du deep learning dans ce contexte.

La deuxième limite se trouve au niveau des ressources matérielles. Comme évoqué plus tôt, les ressources de calcul ne sont ni illimitées ni toujours accessibles. Les pannes ou périodes d'inaccessibilité de Glicid ont régulièrement imposé de réorienter le travail vers d'autres tâches : bibliographie, rédaction, analyse, tout ce qui ne nécessite pas de ressources de calcul importantes.
...
Face aux limites de temps et de disponibilité des outils, des choix ont été faits, privilégier des tests plus simples mais solides pour parvenir à des premiers résultats évaluables, plutôt que de s'éparpiller sur des pistes trop nombreuses.
...
Le codage vidéo repose sur un grand nombre de principes parfois complexes pour réduire l'information transmise, le but ici sera seulement de donner les logiques de base et certains éléments importants qui permettront une meilleure compréhension du sujet et faciliteront l'identification des éléments d'une possible amélioration.
Une vidéo est une suite d'images qui défilent, souvent très rapidement (plusieurs par seconde). On peut parfois avoir des vidéos qui contiennent 30, 60, voire 120 images par seconde (#gls("fps", "FPS")), et plus dans certains cas, pour des types de contenus qui demandent une grande fluidité. Durant un court laps de temps, la scène ne change que très peu. C'est le point principal utilisé pour la compression vidéo, exploiter cette redondance d'informations de manière efficace.

Pour bien comprendre les différents concepts, il faut aussi avoir en tête qu'une image est décomposée en petits blocs, qui peuvent être de taille variable pour une même image. Dans les exemples, nous garderons une taille fixe pour faciliter la compréhension.
...
L'œil étant beaucoup plus sensible à la luminance qu'à la couleur, on peut alors sous-échantillonner les informations liées à la couleur. On retrouve notamment le format 4:2:0, souvent utilisé pour les vidéos, où la couleur est transmise à une résolution réduite par rapport à la luminance. La perte est réelle, mais quasiment imperceptible.
...
On voit alors que malgré une réduction importante des données pour le format 4:2:0, qui supprime 75 % des informations de couleurs, on retrouve une image qui reste assez similaire à celle contenant toutes les informations (le format 4:4:4). Un autre exemple met en avant cette efficacité, avec cette méthode (4:2:0) on garde 6 unités sur 12 (4 de luminance + 1 + 1 de chrominance), soit 50 %, on a donc réduit par deux les données sans perdre trop en qualité perçue.

Ces deux exemples montrent une chose importante pour la suite : la compression ne cherche pas la fidélité parfaite, mais la fidélité perçue. C'est exactement le terrain sur lequel se place ce projet, modifier l'image pour qu'elle coûte moins cher à coder, sans que l'utilisateur final ne le remarque.

== Quantité d'informations des vidéos et transfert : logique et théorie <th_info>
...
@codec_pipeline les remet dans l'ordre : de la vidéo brute au flux compressé
(encodeur, colonne de gauche), puis du flux reçu à l'image reconstituée
(décodeur, colonne de droite), afin de visualiser l'enchaînement complet et simplifié d'un
#gls("codec", "codec") tel que #gls("hevc", "H.265").

#figure(
  scale(x: 88%, y: 88%, reflow: true, {
    set text(font: "Liberation Sans", fill: rgb("#1a2530"))
    let ENC_BAND = rgb("#e8f1fb")
    let ENC_HEAD = rgb("#1f6fb2")
    let ENC_STROKE = rgb("#1f6fb2")
    let DEC_BAND = rgb("#e7f6ec")
    let DEC_HEAD = rgb("#2e9e5b")
    let DEC_STROKE = rgb("#2e9e5b")
    let ORANGE = rgb("#e8763a")
    let INK = rgb("#1a2530")
    ...
    canvas(length: 1cm, {
      ...
      // Helper closures
      // ============================================================
      let W = 4.3 // stage box width
      let H = 2.6 // stage box height

      // A stage box: cx = center x, top = top y, drawing icon + title
      let stage = (cx, top, stroke-col, title, icon) => {
        let l = cx - W / 2
        let r = cx + W / 2
        let b = top - H
        rect((l, b), (r, top), radius: 0.14, fill: white, stroke: stroke-col + 1.4pt)
        // icon zone (upper part)
        icon(cx, top - 0.92)
        // title (lower part)
        content((cx, top - 2.15), box(width: (W - 0.2) * 1cm)[
          #set text(size: 8.5pt, weight: "bold")
          #align(center)[#title]
        ])
      }

      // caption under a box (anchored at its TOP so text grows downward)
      let caption = (cx, y, body) => {
        content(
          (cx, y),
          box(width: (W + 0.5) * 1cm)[
            #set text(size: 6.8pt, fill: rgb("#3a4550"))
            #align(center)[#body]
          ],
          anchor: "north",
        )
      ...
      // arrow
      let arr = (a, b, ..o) => {
        line(a, b, stroke: INK + 1.6pt, mark: (end: ">", fill: INK), ..o)
      }

      // ---------- Icons (each: (cx, cy) center) ----------
      let ic-frames = (cx, cy, col) => {
        for i in (0, 1, 2) {
          let dx = 0.12 * (2 - i)
          let dy = 0.12 * (2 - i)
          rect((cx - 0.55 + dx, cy - 0.42 + dy), (cx + 0.35 + dx, cy + 0.42 + dy), fill: white, stroke: col + 0.9pt)
        }
        // little image inside front frame
        rect((cx - 0.55, cy - 0.42), (cx + 0.35, cy + 0.42), fill: rgb("#cfe6ff"), stroke: col + 0.9pt)
        line(
          (cx - 0.55, cy - 0.05),
          (cx - 0.15, cy + 0.18),
          (cx + 0.05, cy - 0.02),
        ...
        circle((cx + 0.12, cy + 0.22), radius: 0.07, fill: ORANGE, stroke: none)
      }

      let ic-blocks = (cx, cy, col) => {
        let s = 0.82
        // faint "image" background so blocks read as a picture cut up
        rect((cx - s, cy - s), (cx + s, cy + s), fill: rgb("#eaf3fc"), stroke: col + 1.2pt)
        // flat area on the left = one big block; detailed area on the right
        rect((cx - s, cy - s), (cx, cy + s), fill: rgb("#cfe6ff"), stroke: col + 1pt)
        // right half split into 4, then top-right split into 4 again (quadtree)
        line((cx, cy - s), (cx, cy + s), stroke: col + 1pt)
        line((cx, cy), (cx + s, cy), stroke: col + 0.9pt)
        line((cx + s / 2, cy), (cx + s / 2, cy + s), stroke: col + 0.9pt)
        // fine subdivision (detail) in the top-right quadrant
        line((cx + s / 2, cy + s / 2), (cx + s, cy + s / 2), stroke: col + 0.7pt)
        line((cx + 3 * s / 4, cy + s / 2), (cx + 3 * s / 4, cy + s), stroke: col + 0.7pt)
        // shade the small detailed cells
        rect((cx + s / 2, cy + s / 2), (cx + 3 * s / 4, cy + s), fill: rgb("#9cc7f0"), stroke: col + 0.6pt)
        rect((cx + 3 * s / 4, cy + s / 2), (cx + s, cy + s), fill: rgb("#7dB6e8"), stroke: col + 0.6pt)
      }

      let ic-predict = (cx, cy, col) => {
        // ---- Panel 1 : INTRA (predict from neighbours inside the SAME image) ----
        let lx = cx - 0.78
        let g = 0.24
        // 3x3 block grid
        for i in range(3) {
          for j in range(3) {
            rect(
              (lx - 1.5 * g + i * g, cy - 0.05 - 1.5 * g + j * g),
              (lx - 1.5 * g + (i + 1) * g, cy - 0.05 - 1.5 * g + (j + 1) * g),
              fill: white,
              stroke: col + 0.5pt,
            )
          }
        }
        // already-decoded neighbours: top row + left column (shaded)
        for i in range(3) {
          rect(
            (lx - 1.5 * g + i * g, cy - 0.05 + 0.5 * g),
            (lx - 1.5 * g + (i + 1) * g, cy - 0.05 + 1.5 * g),
            fill: rgb("#bcd8f2"),
            stroke: col + 0.5pt,
          )
        }
        for j in range(2) {
          rect(
            (lx - 1.5 * g, cy - 0.05 - 1.5 * g + j * g),
            (lx - 1.5 * g + g, cy - 0.05 - 1.5 * g + (j + 1) * g),
            fill: rgb("#bcd8f2"),
            stroke: col + 0.5pt,
          )
        }
        // target block (bottom-right cell) predicted from those neighbours
        rect(
          (lx + 0.5 * g, cy - 0.05 - 1.5 * g),
          (lx + 1.5 * g, cy - 0.05 - 0.5 * g),
          fill: rgb("#fbe3d2"),
          stroke: ORANGE + 0.8pt,
        )
        line((lx + g, cy - 0.05 + g), (lx + g, cy - 0.05 - 0.6 * g), stroke: ORANGE + 0.9pt, mark: (
          end: ">",
          fill: ORANGE,
          scale: 0.5,
        ))
        line((lx - g, cy - 0.05 - g), (lx + 0.4 * g, cy - 0.05 - g), stroke: ORANGE + 0.9pt, mark: (
          end: ">",
          fill: ORANGE,
          scale: 0.5,
        ))
        content((lx, cy + 0.62), text(size: 6pt, weight: "bold", fill: col)[1 · intra])

        // ---- Panel 2 : INTER (predict from a reference image + motion) ----
        let rx = cx + 0.82
        // reference frame (behind)
        rect((rx - 0.42, cy - 0.42), (rx + 0.18, cy + 0.18), fill: rgb("#dcebfb"), stroke: col + 0.6pt)
        rect((rx - 0.32, cy - 0.28), (rx - 0.02, cy + 0.02), fill: rgb("#9cc7f0"), stroke: col + 0.5pt)
        // current frame (front)
        rect((rx - 0.18, cy - 0.18), (rx + 0.42, cy + 0.42), fill: white, stroke: col + 0.8pt)
        rect((rx + 0.06, cy + 0.06), (rx + 0.36, cy + 0.36), fill: rgb("#fbe3d2"), stroke: ORANGE + 0.7pt)
        // motion vector
        line((rx - 0.17, cy - 0.13), (rx + 0.2, cy + 0.2), stroke: ORANGE + 1pt, mark: (
          end: ">",
          fill: ORANGE,
          scale: 0.6,
        ))
        content((rx, cy + 0.62), text(size: 6pt, weight: "bold", fill: col)[2 · inter])

        // order arrow intra -> inter
        line((lx + 1.5 * g + 0.06, cy - 0.05), (rx - 0.5, cy - 0.05), stroke: rgb("#9aa5b0") + 0.9pt, mark: (
          end: ">",
          fill: rgb("#9aa5b0"),
          scale: 0.6,
        ))
      ...
      let ic-sign = (cx, cy, col, sym) => {
        circle((cx, cy), radius: 0.42, fill: white, stroke: ORANGE + 1.4pt)
        // minus / plus
        line((cx - 0.22, cy), (cx + 0.22, cy), stroke: ORANGE + 1.8pt)
        if sym == "+" {
          line((cx, cy - 0.22), (cx, cy + 0.22), stroke: ORANGE + 1.8pt)
        }
      }

      // small image thumbnail: kind in ("img", "pred", "res")
      let thumb = (cx, cy, kind) => {
        let w = 0.54
        if kind == "res" {
          // residual: nearly flat grey, only faint edges remain
          rect((cx - w / 2, cy - w / 2), (cx + w / 2, cy + w / 2), fill: rgb("#e9ebee"), stroke: INK + 0.6pt)
          line((cx - 0.14, cy + 0.1), (cx + 0.02, cy - 0.06), stroke: rgb("#9aa5b0") + 0.8pt)
          line((cx - 0.02, cy - 0.12), (cx + 0.16, cy + 0.04), stroke: rgb("#9aa5b0") + 0.7pt)
        } else {
          // sky + ground + sun (an actual little picture)
          let sky = if kind == "pred" { rgb("#cfe6ff") } else { rgb("#bfe0ff") }
          let ground = if kind == "pred" { rgb("#a9cf96") } else { rgb("#7fae6a") }
          let sun = if kind == "pred" { rgb("#f0b48a") } else { ORANGE }
          rect((cx - w / 2, cy), (cx + w / 2, cy + w / 2), fill: sky, stroke: none)
          rect((cx - w / 2, cy - w / 2), (cx + w / 2, cy), fill: ground, stroke: none)
          let sx = if kind == "pred" { cx + 0.04 } else { cx + 0.11 }
          circle((sx, cy + 0.13), radius: 0.06, fill: sun, stroke: none)
        }
        rect((cx - w / 2, cy - w / 2), (cx + w / 2, cy + w / 2), fill: none, stroke: INK + 0.7pt)
      }

      // image A (op) B = C
      let ic-imgop = (cx, cy, aKind, sym, bKind, cKind) => {
        thumb(cx - 1.12, cy, aKind)
        content((cx - 0.62, cy), text(size: 11pt, weight: "bold", fill: ORANGE)[#sym])
        thumb(cx - 0.12, cy, bKind)
        content((cx + 0.4, cy), text(size: 10pt, weight: "bold", fill: INK)[=])
        thumb(cx + 0.92, cy, cKind)
      }

      let ic-dct = (cx, cy, col, c0, c1, c2) => {
        // frequency grid: strongest coefficients concentrated in TOP-LEFT
        let n = 4
        let s = 0.32
        let x0 = cx - (n * s) / 2
        let y0 = cy - (n * s) / 2
        for i in range(n) {
          for j in range(n) {
            // distance from the top-left corner (i=0 column, j=n-1 top row)
            let d = i + (n - 1 - j)
            let fill = white
            if d == 0 { fill = c0 } else if d == 1 { fill = c1 } else if d == 2 { fill = c2 }
            rect((x0 + i * s, y0 + j * s), (x0 + (i + 1) * s, y0 + (j + 1) * s), fill: fill, stroke: col + 0.6pt)
          }
        }
        // hint: low freq (top-left) -> high freq (bottom-right)
        content((x0 - 0.02, y0 + n * s + 0.16), text(size: 5.5pt, fill: rgb("#7a848e"))[basses f.])
      }

      // list of "left -> right" mappings; rows = ((left, right, isZero), ...)
      let ic-maplist = (cx, cy, col, rows) => {
        let n = rows.len()
        let step = 0.36
        let y0 = cy + (n - 1) * step / 2
        for k in range(n) {
          let row = rows.at(k)
          let yy = y0 - k * step
          let rc = if row.at(2) { rgb("#9aa5b0") } else { col }
          content((cx - 0.55, yy), text(size: 7.5pt, fill: INK)[#row.at(0)])
          content((cx - 0.02, yy), text(size: 7.5pt, fill: rgb("#9aa5b0"))[$arrow.r$])
          content((cx + 0.5, yy), text(size: 7.5pt, weight: "bold", fill: rc)[#row.at(1)])
        }
      }

      let ic-entropy = (cx, cy, col, leafcol, bits) => {
        // compact tree with 0 / 1 edge labels
        let root = (cx, cy + 0.48)
        let l = (cx - 0.5, cy + 0.02)
        let r = (cx + 0.5, cy + 0.02)
        let rl = (cx + 0.25, cy - 0.42)
        let rr = (cx + 0.75, cy - 0.42)
        for (a, b) in ((root, l), (root, r), (r, rl), (r, rr)) {
          line(a, b, stroke: col + 0.8pt)
        }
        content(((root.at(0) + l.at(0)) / 2 - 0.1, (root.at(1) + l.at(1)) / 2 + 0.05), text(
          size: 5.5pt,
          fill: rgb("#7a848e"),
        )[0])
        content(((root.at(0) + r.at(0)) / 2 + 0.1, (root.at(1) + r.at(1)) / 2 + 0.05), text(
          size: 5.5pt,
          fill: rgb("#7a848e"),
        )[1])
        circle(root, radius: 0.13, fill: col, stroke: none)
        circle(r, radius: 0.12, fill: col, stroke: none)
        for p in (l, rl, rr) { circle(p, radius: 0.11, fill: leafcol, stroke: none) }
        // resulting bitstream
        content((cx, cy - 0.72), text(size: 7.5pt, font: "Liberation Mono", weight: "bold", fill: col)[#bits])
      }

      let ic-smooth = (cx, cy, col) => {
        rect((cx - 0.7, cy - 0.55), (cx + 0.7, cy + 0.55), fill: rgb("#d8f0e0"), stroke: col + 0.9pt)
        // wavy smoothing line
        line(
          (cx - 0.6, cy - 0.15),
          (cx - 0.3, cy + 0.25),
      ...
      let ic-file = (cx, cy, col) => {
        rect((cx - 0.5, cy - 0.6), (cx + 0.45, cy + 0.6), fill: white, stroke: col + 1.1pt)
        // folded corner
        line((cx + 0.2, cy + 0.6), (cx + 0.45, cy + 0.35), stroke: col + 1.1pt)
        for k in (0.28, 0.05, -0.18, -0.4) {
          line((cx - 0.32, cy + k), (cx + 0.27, cy + k), stroke: col + 0.7pt)
        }
      }

      let ic-monitor = (cx, cy, col) => {
        rect((cx - 0.75, cy - 0.35), (cx + 0.75, cy + 0.55), fill: rgb("#dBeafe"), stroke: col + 1.1pt)
        line(
          (cx - 0.5, cy + 0.05),
          (cx - 0.1, cy + 0.3),
          (cx + 0.15, cy + 0.1),
        ...
        circle((cx + 0.2, cy + 0.35), radius: 0.06, fill: ORANGE, stroke: none)
        line((cx - 0.2, cy - 0.35), (cx - 0.2, cy - 0.55), stroke: col + 1.1pt)
        line((cx + 0.2, cy - 0.35), (cx + 0.2, cy - 0.55), stroke: col + 1.1pt)
        line((cx - 0.55, cy - 0.55), (cx + 0.55, cy - 0.55), stroke: col + 1.4pt)
      }

      // ============================================================
      // Layout (VERTICAL : deux colonnes en U, bandeaux horizontaux)
      // ============================================================
      let pitch = 3.15
      let TOP = 22.0
      let ex = 3.9 // colonne encodeur (descend)
      let dx = 11.6 // colonne décodeur (remonte)

      let ey = i => TOP - i * pitch // haut de la boîte encodeur i (0..5)
      let dtop = s => TOP - (4 - s) * pitch // haut de la boîte décodeur d'étape s (0..4)

      let midy = 20.7 // hauteur du centre de la 1re boîte (entrée / sortie)

      // ----- Bandes de fond + bandeaux HORIZONTAUX en haut -----
      let headerTop = TOP + 1.7
      let bodyTop = TOP + 0.65
      let encBot = ey(5) - H - 0.55
      let decBot = dtop(0) - H - 0.55
      // encodeur
      rect((1.5, encBot), (6.3, headerTop), radius: 0.25, fill: ENC_BAND, stroke: ENC_STROKE + 1pt)
      rect((1.5, bodyTop), (6.3, headerTop), radius: (north: 0.25), fill: ENC_HEAD, stroke: none)
      content((3.9, (bodyTop + headerTop) / 2), text(size: 10.5pt, weight: "bold", fill: white)[ENCODEUR VIDÉO])
      // décodeur
      rect((9.2, decBot), (14.0, headerTop), radius: 0.25, fill: DEC_BAND, stroke: DEC_STROKE + 1pt)
      rect((9.2, bodyTop), (14.0, headerTop), radius: (north: 0.25), fill: DEC_HEAD, stroke: none)
      content((11.6, (bodyTop + headerTop) / 2), text(size: 10.5pt, weight: "bold", fill: white)[DÉCODEUR VIDÉO])

      // ----- Étapes ENCODEUR (haut -> bas) -----
      stage(ex, ey(0), ENC_STROKE, [Partitionnement en blocs], (x, y) => ic-blocks(x, y, ENC_STROKE))
      stage(ex, ey(1), ENC_STROKE, [Prédiction Intra puis Inter], (x, y) => ic-predict(x, y, ENC_STROKE))
      stage(ex, ey(2), ENC_STROKE, [Traitement du résidu], (x, y) => ic-imgop(x, y, "img", "−", "pred", "res"))
      stage(ex, ey(3), ENC_STROKE, [Transformation (DCT)], (x, y) => ic-dct(
        x,
        y,
        ENC_STROKE,
        rgb("#1f6fb2"),
      ...
      stage(ex, ey(4), ENC_STROKE, [Quantification], (x, y) => ic-maplist(x, y, ENC_STROKE, (
        ("7.6", "8", false),
        ("3.2", "3", false),
        ("0.4", "0", true),
        ("0.1", "0", true),
      )))
      stage(ex, ey(5), ENC_STROKE, [Encodage entropique], (x, y) => ic-entropy(
        x,
        y,
        ENC_STROKE,
        rgb("#6aa8dc"),
      ...
      for i in range(5) {
        arr((ex, ey(i) - H), (ex, ey(i + 1)))
      }

      // ----- Étapes DÉCODEUR (bas -> haut) -----
      stage(dx, dtop(0), DEC_STROKE, [Décodage entropique], (x, y) => ic-entropy(
        x,
        y,
        DEC_STROKE,
        rgb("#7cc795"),
      ...
      stage(dx, dtop(1), DEC_STROKE, [Déquantification], (x, y) => ic-maplist(x, y, DEC_STROKE, (
        ("8", "8.0", false),
        ("3", "3.0", false),
        ("0", "0.0", true),
      )))
      stage(dx, dtop(2), DEC_STROKE, [Transformation inverse], (x, y) => ic-dct(
        x,
        y,
        DEC_STROKE,
        rgb("#2e9e5b"),
      ...
      stage(dx, dtop(3), DEC_STROKE, [Reconstruction], (x, y) => ic-imgop(x, y, "res", "+", "pred", "img"))
      stage(dx, dtop(4), DEC_STROKE, [Post-traitement], (x, y) => ic-smooth(x, y, DEC_STROKE))
      for s in range(4) {
        arr((dx, dtop(s)), (dx, dtop(s + 1) - H))
      }

      // ----- Entrée : vidéo brute (à gauche, entre dans la 1re boîte) -----
      ic-frames(0.15, midy, INK)
      content((0.15, midy - 1.15), box(width: 2.4cm)[
        #set text(size: 7.5pt, weight: "bold")
        #align(center)[VIDÉO BRUTE
          #linebreak() #text(weight: "regular", size: 6.6pt)[(Séquence d'images)]]
      ])
      arr((0.8, midy), (ex - W / 2, midy))

      // ----- Sortie : vidéo reconstituée (à droite) -----
      ic-monitor(15.1, midy, INK)
      content((15.1, midy - 1.2), box(width: 2.6cm)[
        #set text(size: 7.5pt, weight: "bold")
        #align(center)[VIDÉO#linebreak()RECONSTITUÉE
          #linebreak() #text(weight: "regular", size: 6.6pt)[(Visible par l'utilisateur)]]
      ])
      arr((dx + W / 2, midy), (14.35, midy))

      // ----- Pont bas : flux compressé + stockage -----
      let by = 2.15
      ic-file(6.6, by + 0.1, INK)
      content((6.6, by - 1.05), box(width: 3cm)[
        #set text(size: 7.5pt, weight: "bold")
        #align(center)[FLUX COMPRESSÉ]
      ])
      rect((8.0, by - 0.55), (10.5, by + 0.55), radius: 0.12, fill: rgb("#eef1f4"), stroke: INK + 1.2pt)
      content((9.25, by), box(width: 2.3cm)[
        #set text(size: 7.5pt, weight: "bold")
        #align(center)[STOCKAGE /\ TRANSMISSION]
      ])
      arr((ex, ey(5) - H), (6.05, by + 0.25))
      arr((7.2, by), (8.0, by))
      arr((10.5, by + 0.15), (dx, dtop(0) - H))
    })
  }),
  caption: [Déroulement complet d'un #gls("codec", "codec"), de la vidéo brute au flux compressé (encodeur, colonne de gauche) puis du flux reçu à l'image reconstituée (décodeur, colonne de droite).],
) <codec_pipeline>

== Évaluer le contenu vidéo <contenteval>

Si l'on veut optimiser une vidéo, encore faut-il pouvoir mesurer sa qualité. C'est un point central du projet, pour deux aspects, la mesure choisie servira de guide à l'apprentissage du filtre, elle se doit donc d'être un critère qui reflète la vision humaine, et durant l'évaluation, cela conditionnera aussi la pertinence des résultats obtenus.

La référence reste le jugement humain. En réunissant un panel d'observateurs et en moyennant leurs notes, on obtient un #gls("mos", "MOS") (_Mean Opinion Score_), considéré comme la "vérité terrain" de la qualité perçue. C'est d'ailleurs l'une des expertises reconnues de notre équipe, sollicitée pour ce type de tests par de grands acteurs du secteur. Mais ces tests sont coûteux et lents : impossible de les utiliser pour guider, image par image, l'entraînement d'un réseau de neurones qui nécessite un grand nombre d'exemples.
...
Nous l'avons mentionné, les codecs classiques s'intègrent mal dans une chaîne d'apprentissage automatique. Avant de voir comment contourner ce problème, il faut comprendre d'où viennent ces limitations, en reprenant les étapes clés qui, dans leur version d'origine, ne peuvent pas faire partie d'un environnement d'apprentissage d'un réseau de neurones.

=== Limites mathématiques des outils pour l'apprentissage <limites_codec>

Ces limites sont avant tout mathématiques. Pour qu'un réseau de neurones, dans notre cas, un filtre, apprenne, chaque opération de la boucle doit indiquer dans quelle direction ajuster les paramètres, c'est le rôle du "gradient", qui donne en quelque sorte le sens de la pente. Une opération est utilisable pour l'apprentissage si elle est différentiable, c'est-à-dire une fonction suffisamment régulière, sans cassure et dont on peut mesurer la pente.
...
Le défi majeur se trouve donc dans la création d'une version repoussant ces limites, mais qui s'approche de la réalité, ce qui est très complexe notamment de par les optimisations poussées des codecs, des optimisations compliquées à reproduire d'autant plus qu'elles sont souvent non optimisables.


== Solutions existantes

La mise en place d'un terrain d'optimisation fiable : un #gls("proxy", "proxy") (jumeau du codec cible) est un des piliers du projet, c'est lui qui permettra d'entraîner correctement le filtre et, plus tard, possiblement d'autres outils destinés à assister la compression. Tester différentes méthodologies de proxy était donc un premier point d'étape essentiel, cela s'intègre naturellement dans le cadre d'un projet de fin d'études. La partie scientifique se penchera donc en particulier sur ce point en apportant aussi d'autres aspects du projet mais à ce stade précoce, il est d'abord essentiel de comprendre les différentes possibilités avant de définir la méthode qui sera utilisée pour l'implémentation finale.

Dans cette section, nous verrons différentes solutions existantes qui serviront de références techniques pour les tests mis en place.
...
Ce graphe se parcourt dans les deux sens. À l'aller (le #gls("fwbw", "forward")), on calcule normalement le résultat à partir des entrées. Au retour (le #gls("fwbw", "backward")), on remonte le graphe en sens inverse pour calculer, pour chaque opération, le gradient, c'est-à-dire la contribution de chacune des opérations à l'erreur finale qui remonte ce graphe. C'est cette seconde étape qui permet d'ajuster le réseau et c'est ici qu'il est nécessaire que les opérations mathématiques soient dérivables sinon l'erreur ne pourra pas être corrigée (le gradient sera bloqué).

Séparer ces deux étapes ouvre une possibilité intéressante : on peut intervenir sur le #gls("fwbw", "backward") indépendamment du #gls("fwbw", "forward"). Deux outils en découlent, le premier consiste à "détacher" une partie du graphe, de sorte que l'opération ne soit pas prise en compte : la valeur est bien calculée à l'aller, mais on ne remonte pas par ce chemin. La @forward_backward illustre ce principe sur un calcul simple.

Le second, plus puissant, consiste à remplacer le comportement d'une fonction au #gls("fwbw", "backward"). On exécute l'opération réelle à l'aller, mais on la remplace par une approximation optimisable au retour. C'est le principe du #gls("ste", "STE") (_Straight Through Estimator_), pour un arrondi, par exemple, on applique le vrai arrondi au #gls("fwbw", "forward"), mais on fait "comme si" la fonction était différente au #gls("fwbw", "backward"), on utilise alors une fonction dérivable, ce qui laisse passer un gradient exploitable.
...
*Limites.* L'implémentation paraît simple, mais cette simplicité vient surtout de la difficulté à reproduire fidèlement un codec complexe tout en gardant des fonctions dérivables et compatibles avec la mémoire des cartes graphiques (la recherche de blocs est notamment coûteuse en mémoire). Par ailleurs, l'estimation du débit ne repose pas sur un calcul figé : elle s'appuie sur un modèle d'entropie appris, ce qui réintroduit une composante apprise là où l'on cherchait justement à s'affranchir de composantes apprises par IA et à reproduire la logique du codage théorique classique.

*Sandwiched Video Compression (Google, ICIP 2023) — codec simplifié avec encadrement pré/post* @isik2023sandwiched

Cette approche reprend l'idée du "sandwich" ou similaire au "neural wrapper" vu avant : un filtre de prétraitement et un filtre de post-traitement, très légers, encadrent un #gls("codec", "codec") standard. Le proxy de codec est, comme pour l'exemple précédent, une version optimisable simplifiée : codage intra de type image (#gls("dct", "DCT") et quantification avec un estimateur de type _straight-through_, #gls("ste", "STE")), compensation de mouvement, sélection de mode inter/intra, codage du résidu, filtre de boucle, et une estimation de débit différentiable et simple. Cette approche obtient dans certains cas des gains aux alentours de 30 % de débit économisé sous la métrique perceptuelle #gls("lpips", "LPIPS").

*Limites.* L'évaluation porte sur #gls("hevc", "H.265") et sur des scénarios assez spécifiques, principalement avec le #gls("psnr", "PSNR") et #gls("lpips", "LPIPS"), ce qui rend la transposition directe à notre cas moins évidente. Et là encore, le post-traitement côté client pose les mêmes difficultés de déploiement. On peut d'ailleurs imaginer que les résultats sont en bonne partie dus à ce filtre de post-traitement, comme pour le premier papier présenté.
...
Concrètement, notre implémentation poursuivait deux objectifs : reproduire un proxy par codage neuronal adapté à #gls("hevc", "H.265"), et réaliser une version simplifiée d'un #gls("codec", "codec") cherchant à se rapprocher de sa logique de compression.
...
Dans ce schéma, le terme #gls("proxy", "proxy") est associé à la copie du codec pour simplifier la compréhension, les scores liés aux #gls("metrique", "métriques") sont aussi remplacés par *résultat du filtre*, car dans cette boucle, les métriques nous servent de guide et définissent les résultats du filtre durant l'apprentissage.

Il est important de différencier le proxy du véritable codec. Pour simplifier, le filtre va agir sur les images qui seront ensuite encodées et décodées par le vrai codec, puis par son proxy, les métriques évalueront les images produites par le véritable codec mais n'étant pas différentiable (pas de pente partout), l'optimisation se fera sur le proxy. Cela permet de s'assurer que le comportement du filtre fonctionne sur le véritable outil cible (#gls("hevc", "H.265")), on contourne alors les limitations grâce à cette méthodologie.
...
Pour rester exécutable sur carte graphique malgré le coût des recherches de blocs, @chadha2021dpp propose une méthodologie reproduisant la complexité des opérations de prédictions pour les images clés, tout en respectant les contraintes mémoire car ce sont des opérations qui sont facilement exécutables sur des cartes graphiques. Nous reprenons alors une logique qui ressemble à la prédiction inter (@intermotion), mais en l'appliquant ici à l'intérieur d'une seule image, la zone de recherche y est restreinte à ce qui est théoriquement déjà connu dans l'image en cours de décodage, c'est-à-dire la partie située au-dessus du bloc courant et à sa gauche (@zonecausale). Cette contrainte est essentielle, un décodeur ne dispose jamais des blocs "futurs", et l'ignorer reviendrait à entraîner le filtre sur une information indisponible à la reconstruction réelle. Cette méthode permet alors de reproduire des transformations complexes pour un coût minime, il serait presque impossible de réaliser des opérations classiques de manière conditionnelle comme le ferait un vrai codec et ces opérations non optimisables mises bout à bout rendraient le terrain d'apprentissage très pauvre. Cette méthode semble donc être un bon compromis entre faisabilité et respect de la complexité des méthodes d'origine. Pour résumé on va donc checher le meilleur bloc dans une zone restreinte, sans ce limiter à des opérations conditionnelles comme une prédiction d'image clé classique. La prédiction des images suivantes se fait d'une manière similaire, mais en prenant en compte l'image précédente pour la zone de recherche (on reste ici très proche du fonctionnement classique).
...
#figure(
  ...
  caption: [Arrondi dur (escalier, en pointillés) et sa version adoucie continue (_arrondi doux_).],
) <softround>

L'arrondi réel a un gradient nul presque partout, la version adoucie suit les mêmes marches tout en gardant une légère pente, ce qui rend cette fonction optimisable.
...
Dans cette implémentation, il est aussi important d'établir la manière dont on va permettre à notre outil d'estimer le coût d'une image.
...
Cette forme, par sa pente, pousse le filtre à réduire l'amplitude des coefficients en particulier quand ils sont déjà proches de 0 (où la pente est plus forte). C'est exactement l'objectif recherché, simplifier l'image pour la rendre moins coûteuse à coder.

Ce choix est fait car obtenir la probabilité d'un coefficient est difficile, encore plus de l'obtenir en répliquant les méthodes d'un véritable codec et rendre ces étapes complexes optimisables, devient alors presque impossible. Cette simplification, imparfaite permet tout de même d'obtenir une estimation proche et reste simple à optimiser.
...
Il est important aussi de préciser que certains tests n'ont pas réellement été des échecs mais la temporalité liée au projet de fin d'études limite le champ d'action, certaines idées ou améliorations prendront alors place pour le futur du projet selon leur pertinence.


=== Bilan de l'implémentation

Cette implémentation s'est révélée particulièrement intéressante pour tester différents mécanismes et mettre en relation la théorie évoquée dans le document avec des cas d'utilisation concrets. Durant son développement, plusieurs essais rapides n'ont apporté aucune amélioration nette, voire ont entraîné de fortes pertes, ce qui a permis de converger vers les choix présentés ci-dessus. L'objectif étant précisément de définir les bonnes méthodes pour fiabiliser un tel outil, des études comparant ces différentes méthodes sont présentées en @resultats.
...
À cela s'ajoute une précaution méthodologique, nous l'avons évoqué dans les limites des travaux existants, certains projets entraînent le filtre sur une métrique puis évaluent les performances sur cette même métrique, ce qui fausse probablement l'analyse. Nous chercherons donc à dissocier le guide d'apprentissage des outils servant à l'évaluation finale.
...
#gls("dists", "DISTS") apporte la dimension perceptuelle. Elle s'appuie sur un réseau de neurones (IA) et est calibrée sur des jugements humains de similarité. Entraînée à partir de paires d'images, elle est théoriquement capable de savoir si une texture différente au niveau des pixels est acceptable pour un utilisateur. Ce qui rend #gls("dists", "DISTS") pertinent ici est donc sa tolérance aux textures. Autrement dit, elle laisse au filtre la liberté de modifier l'image au niveau du détail, sans le pénaliser tant que la satisfaction de l'utilisateur n'est pas affectée. C'est exactement le type de souplesse que l'on recherche pour réduire le débit sans dégrader la qualité perçue. De plus elle a été conçue pour ce type de projet de modification d'images.
...
Concernant l'utilisation d'autres méthodes, différents tests sont en cours mais ne sont pas à un stade assez abouti pour les évoquer dans ce document.


== Méthode d'évaluation

La méthode d'évaluation est un point essentiel du projet. Nous l'avons vu (@contenteval), il existe de nombreux outils prévus pour évaluer la qualité vidéo. L'objectif pour nous est de définir les meilleurs outils pour refléter l'avis d'un utilisateur.
...
En résumé, #gls("vmaf", "VMAF") et UVQ s'imposent comme les métriques principales, tandis que LPIPS pourrait apporter des informations complémentaires par sa pertinence face à un cas d'usage proche.
...
Notre cible étant des vidéos de VOD il est important d'utiliser les outils de manière à refléter ce qu'on pourrait retrouver en situation réelle. Le choix a donc été fait de tester les solutions face à une version optimisée de H.265. Il existe différents modes qui permettent d'encoder et décoder plus ou moins rapidement, ce qui va aussi jouer sur la qualité des vidéos finale mais aussi sur le poids des vidéos.

Ce choix de test semble important pour vérifier la validité d'un tel filtre pour notre cible. Mais par des contraintes liées à la durée d'entraînement, la version utilisée lors de l'apprentissage sera, elle, plus rapide (moins optimisée) car cette différence de temps de traitement devient alors trop importante au vu du nombre de vidéos à traiter durant l'apprentissage.

Ce point sera aussi à étudier par la suite car cela entraîne un écart entre le domaine d'entraînement et le domaine de test.
...
meilleure qualité et plus récentes. Il serait alors possible de remplacer ou de combiner les deux jeux de données pour obtenir plus de données, plusieurs options s'offrent alors. Un tel changement pourrait améliorer la
...
Pour évaluer la fidélité de structure des images, nous l'avons vu, #gls("psnr", "PSNR") reste une métrique simple, peu fiable pour la perception humaine, mais qui est un bon indicateur de la fidélité de reconstruction. Ici l'intérêt est d'estimer les performances des outils pour reproduire ce que fait le vrai codec, l'utiliser reste donc un choix simple et pertinent. #gls("ssim", "SSIM") est lui plus précis et plus proche de la perception humaine et évalue la similarité structurelle. Il est donc intéressant de le croiser avec le PSNR pour avoir une idée plus précise de la qualité des images reconstruites. De plus durant l'entraînement du proxy neuronal, la métrique MSE (Mean Squared Error) est utilisée pour guider l'apprentissage, cette métrique étant directement liée à la logique du PSNR, il est donc logique de la croiser avec le SSIM pour avoir une idée plus précise de la qualité des images reconstruites, sans biais.

Pour rappel plus un score PSNR est haut, plus cette mesure considère l'image comme fidèle à l'originale, aux alentours de 40 une image est considérée comme très fidèle. Concernant SSIM les scores varient entre 0 et 1, plus on se rapproche de 1, plus l'image est considérée comme similaire.

On sépare ici les images intra des images inter car nous utilisons des méthodes qui diffèrent de celles d'un codec classique, cela permet de valider ou d'invalider ces deux étapes.
...
#figure(
  ...
  table(
    ...
    [*Arrondi · softmax*], [*40.41*], [0.9574], [39.52], [0.9507],
    [*Arrondi · argmin*], [39.43], [0.9453], [38.35], [0.9319],
    [*Bruit · softmax*], [37.82], [0.9184], [37.22], [0.9110],
    [*Bruit · argmin*], [37.83], [0.9184], [37.22], [0.9110],
...
#figure(
  ...
  table(
    ...
    inset: 6pt,
    table.header([], [*PSNR intra* ↑], [*SSIM intra* ↑], [*PSNR inter* ↑], [*SSIM inter* ↑]),
    [*Arrondi · softmax*], [*42.70*], [0.9738], [*41.57*], [0.9664],
    [*Arrondi · argmin*], [41.41], [0.9627], [40.00], [0.9480],
    [*Bruit · softmax*], [39.40], [0.9363], [38.66], [0.9272],
    [*Bruit · argmin*], [39.40], [0.9363], [38.66], [0.9272],
...
Parmi nos versions simplifiées, la variante `arrondi · softmax` est la plus proche du vrai codec, les autres réglages s'en éloignent un peu. Le softmax est la fonction que nous avions présentée, elle permet de mélanger les candidats de prédiction plutôt que d'en choisir un seul. Cela a pour effet de lisser légèrement le résultat, ce qui est apprécié par le PSNR notamment, ce qui explique en partie ces résultats. On peut tout de même valider la pertinence du proxy simplifié, car lui n'a pas appris à reproduire fidèlement les images.

Il est intéressant de noter que ces performances sont obtenues à cette qualité qui est haute mais devient moins bonne si l'on choisit de traiter des images de plus basse qualité, c'est ce qui a été remarqué en réalisant d'autres tests. On peut expliquer cela car le proxy simplifié ne reprend pas toutes les logiques d'optimisations complexes du véritable codec, des optimisations qui vont être bénéfiques surtout sur des images de plus basse qualité. Notre simulateur fonctionne donc très bien à haute qualité, moins à basse qualité en ce qui concerne la reproduction d'image. Ce qui montre tout de même sa limite d'action et possiblement un désalignement avec le fonctionnement réel encore important. Ce constat nous a donc obligé à utiliser cet outil avec plus de précaution et rester dans une plage limitée de qualité afin d'éviter de s'éloigner trop de l'outil d'origine.
...
Comme simple imitateur du vrai codec, le proxy neuronal semble le meilleur, il réalise les images les plus ressemblantes et estime bien le poids des fichiers. Rien d'étonnant, puisqu'il a été entraîné pour ça. Son inconvénient : il ne fonctionne qu'à un seul niveau de qualité, celui sur lequel il a été entraîné. On peut aussi prédire que son comportement plus éloigné du paradigme de la compression vidéo classique pourrait le rendre moins efficace si on l'utilise comme environnement pour l'apprentissage d'un filtre.

Notre proxy simplifié, lui, fonctionne comme un vrai codec, il découpe l'image en blocs et les compresse à la manière de #gls("hevc", "H.265"). Cette proximité lui donne de vrais atouts. Il estime le poids des fichiers presque aussi bien que le proxy neuronal, il reste compréhensible, et surtout il s'adapte à tous les niveaux de qualité, même s'il devient moins fidèle quand on descend en basse qualité. Ces points en font donc un bon candidat pour guider l'apprentissage d'un filtre.

De par notre implémentation présentée dans @filtreGlobale, les deux proxy possèdent une architecture commune qui prend la version de #gls("hevc", "H.265") en évaluation de la qualité d'image dans la boucle, et va cependant calculer les poids sur le proxy, ce qui a aussi été expliqué dans le papier @khan2025neural. Cela permet notamment d'éviter de travailler sur un domaine trop éloigné de la cible, les images évaluées sortent alors du vrai codec. Cela signifie donc que la fidélité d'image importe moins directement dans notre architecture, car concernant la manière dont le réseau apprend, les options restent similaires (en backward), c'est le but de cette conception. Le point essentiel se trouve donc plutôt dans l'estimation de débit. Pour le proxy simplifié, les meilleurs scores sont obtenus avec la version "ste-argmin", qui est la version où l'estimation de débit reçoit bien le choix d'un bloc unique et pas un mélange de blocs mais va bien apprendre sur ce mélange de blocs. Pour la suite, fort de ce constat nous testerons alors uniquement le paramètre concernant la modification de l'arrondi, c'est-à-dire le choix entre produire un bruit aléatoire pour simuler la quantification ou réaliser cette quantification et ne pas apprendre sur cette étape. Deux paramètres notés ici "Arrondi" et "Bruit".


== Résultats face aux #gls("metrique", "métriques")
Dans cette section, nous verrons comment les différentes versions ont réagi durant l'apprentissage du filtre et donc les résultats obtenus grâce aux effets du filtre, ce qui permettra d'avoir une idée de la pertinence de chaque méthode.

Pour quantifier le gain d'une optimisation, on utilise le #gls("bdrate", "BD-rate"). L'idée repose sur les courbes débit-distorsion : pour une vidéo, on trace la qualité obtenue en fonction du débit (le poids du fichier vidéo), à plusieurs niveaux de compression. On obtient une courbe pour la version sans filtre et d'autres pour les versions filtrées. Le BD-rate mesure alors l'écart horizontal moyen entre ces deux courbes, c'est-à-dire l'économie de débit à qualité égale. Un BD-rate de −5 % signifie que, pour une même qualité perçue, la version filtrée nécessite en moyenne 5 % de débit en moins. Plus la valeur est négative, meilleure est l'optimisation, une valeur positive indique au contraire une dégradation.

Dans un premier temps voici les courbes qui montrent visuellement les écarts entre les différentes implémentations. Il est important de noter que la métrique LPIPS est orientée différemment des autres, plus le point est bas, plus la qualité prédite est bonne, on retrouve aussi l'information sur cette direction sur chacun des graphiques.
...
Ces courbes montrent bien une forte différence entre la version du filtre obtenu via l'entraînement sur la version neuronale et les deux versions du codec simplifié.
On observe aussi une contradiction VMAF semble apprécier les résultats obtenus par la version "Neuro" alors que les autres métriques non.
Concernant les deux versions du proxy simplifié on peut voir que les différences sont assez minimes.
...
#figure(
  ...
  table(
    ...
    stroke: 0.5pt + rgb("#888"),
    inset: 6pt,
    table.header([*Métrique*], [*Bruit* ↓], [*Arrondi* ↓], [*Neuronal* ↓]),
...
Ce tableau reprend les informations que l'on pouvait lire sur les courbes mais de manière plus précise, ce qui permet de mettre en avant les différences entre les deux versions issues du codec simplifié.
On remarque encore cette contradiction entre VMAF et le reste des métriques où la version "Arrondi" semble satisfaire majoritairement VMAF en comparaison à la version "Bruit" qui semble satisfaire les autres mesures.
Cependant ces résultats montrent tout de même que la majorité des métriques semblent apprécier les effets du filtre obtenu par les version "Bruit" et "Arrondi" sur les images, en particulier à basse qualité (les points à gauche dans les graphiques précédents).
Mais les contradictions entre ces différentes mesures montrent la difficulté d'analyse des résultats pour notre cas d'usage. Il semble aussi important de préciser que les différences étant faibles, on peut aussi imaginer que ces écarts se trouvent parfois dans la zone d'erreur de ces différentes mesures, ce qui remet en question ces résultats au vu de la faible différence.

Ces résultats proviennent de la moyenne des différents contenus, mais les différentes vidéos ne réagissent pas de la même manière face au filtre.
Pour illustrer cela voici un exemple d'une vidéo qui semble avoir fonctionné sur les différentes mesures étudiées.
...
Cet exemple montre aussi que les résultats dépendent grandement du contenu, ce qui prouve l'importance des données d'entraînement, qui doivent être représentatives d'une large diversité de contenus afin d'obtenir un outil polyvalent.
...
Dans ces images, on retrouve l'image source, et les différentes versions ainsi que la taille de l'image en kilo-octets, une mesure informatique, plus la valeur est basse, plus la vidéo est légère.
...
On retrouve ici l'exemple où les différentes métriques se sont accordées pour un gain, on voit que le filtre a permis d'éviter d'avoir un bruit de compression important, visuellement on peut aussi confirmer ce gain.
...
#align(center)[
  #figure(
    image("images/exemplvisu/SRC28CRF27.png", width: 110%),
    caption: [Exemple CRF 27 (qualité moyenne)],
  ) <crf27src28>
]

Cet exemple est aussi parlant (mais sombre, donc parfois difficile à observer), la métrique UVQ semble en désaccord avec le gain visuel, le reste
des métriques s’accordent pour un gain minime, seul LPIPS donne un score bien plus important.
...
On voit ici que le filtre "Neuro" apporte un flou important, cependant la métrique VMAF y voit un gain, ce qui montre une incohérence avec le résultat visuel.

Ces exemples montrent qu'il existe parfois des contradictions dans les résultats, visuellement la qualité semble avoir été mieux conservée et le poids de la vidéo diminué, mais certaines métriques ne s'accordent pas sur ce résultat. Ce qui semble normal, la différence étant faible et ces mesures étant imparfaites, il se peut qu'elles se trompent parfois. On peut donc difficilement conclure à partir des résultats actuels. Les quelques exemples montrent tout de même que le filtre peut avoir un intérêt, et le fait qu'un bruit de compression soit évité montre bien que les outils fonctionnent. Cela indique que l'image a été simplifiée, ce qui a permis que l'étape de quantification, qui simplifie l'information, n'ait pas eu un effet trop délétère, ce qui est bien plus visible à basse qualité.

== Limites et perspectives

Les résultats ne permettent pas d'établir une optimisation claire, visuellement certains exemples montrent quelques modifications qui semblent pertinentes quand on regarde les détails. Au regard des exemples visuels et résultats des métriques, on peut donc penser qu'à basse qualité le filtre a un intérêt pour conserver certaines textures ou limiter le bruit de compression.
Nous sommes encore à un stade précoce du projet et ne pas avoir de résultats clairs semble normal, l'objectif ici était aussi de mettre en avant les différences entre les outils que l'on a mis en place et de voir comment ils se comportent.
...
Les deux options offrent tout de même des possibilités différentes. Le proxy simplifié pourra largement être modifié afin d'obtenir des performances plus intéressantes en repliquant par exemple des méthodes plus avancées, mais les scores obtenus pour sa fidélité d'image montrent tout de même que l'implémentation et les choix sont valides. La version "Neuro" semble détruire plus facilement les détails mais donc aussi réduire le poids de manière plus importante, il serait possible de prendre ce résultat en compte et d'adapter certains critères pour éviter cela. Sa fidélité de reconstruction d'image montre que ce choix reste aussi intéressant. Enfin les différences liées aux deux options retenues ("Arrondi" et "Bruit") ne sont pas significatives, il est donc difficile d'établir quel choix est le meilleur.
...
Ce travail s'accompagne d'un constat important : la qualité d'une optimisation dépend de nombreux éléments, le proxy, la #gls("metrique", "métrique") qui guide l'apprentissage, mais aussi les données utilisées ce qui rend le champ des possibilités immense.
...
de dégager les directions qui guideront la poursuite du projet. C'est un travail de
...
/ CRF / QP <crf-qp>: *Constant Rate Factor / Quantization Parameter.* Paramètres réglant l'intensité de la compression : plus leur valeur est élevée, plus la quantification est forte, donc plus la vidéo est légère mais dégradée. Le QP fixe un niveau de quantification rigide, ce qui aura un impact différent en fonction des zones de l'image. Le CRF vise une qualité constante, en essayant donc d'adapter la quantification à la spécificité de la zone.
...
#set heading(
  supplement: none, // pour que @planning affiche « Annexe B » et non « Chapitre Annexe B »
  numbering: (..n) => {
    let p = n.pos()
    if p.len() == 1 { "Annexe " + numbering("A", p.first()) } else { numbering("A.1", ..p) }
  },
)

= Étude sur les plus gros challenges du secteur VOD/Streaming <challengesVOD>
#align(center)[
  #figure(
...
= Architecture du filtre neuronal <archi>
